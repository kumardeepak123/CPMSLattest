...
const EditClient=()=>{
  ...
  const loadAssignedProjects=async()=>{
    await fetch(`https://localhost:44327/api/Project/projects-under-client/${id}`,{
          headers:{
            Authorization: `Bearer ${user.token}`
          }
        })
    ...
    const EditClientHandle= (e)=>{
      ...
      if(selectedOption.length === 0)
      {
        ...
        return;
            ...
            fetch(`https://localhost:44327/api/Client/update-client/${client.id}?ProjectIds=${projectIds}`,{
            method:'PUT',
            headers:{
                "Authorization" : `Bearer ${user.token}`
            },

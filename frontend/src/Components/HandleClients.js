     ...
     const deleteClientHandle=(clientID,clientName)=>{
        if(!window.confirm("Do you really want to delete?")){
          return;
            ...
            fetch(`https://localhost:44327/api/Client/delete-client/${clientID}`,{
              method:'DELETE',
              headers:{
      ...
      const searchEmployee=()=>{
        ...
        fetch(`https://localhost:44327/api/Client/all-clients?sortBy=${sortBy}&&orderBy=${orderBy}&&searchByName=${searchBy}`,{
            headers:{
                "Authorization" : `Bearer ${user.token}`
            }
        })
